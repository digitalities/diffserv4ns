...
#
# DISCLAIMER: This is the ns-2.35 *per-flow srTCM* variant of thesis
# Scenario 2 (Section 4.2 — "full-scale web traffic scenario"). It is a
# minimal-diff fork of scenario-2-ns235.tcl whose only difference is the
# edge classifier: the three DSCP-only `Dumb` policy/policer blocks for
# DSCP 10/12/14 are replaced with per-flow srTCM policy entries (one
# per Telnet / FTP / HTTP connection).
#
# This scenario depends on the 2026 per-flow `PolicyClassifier`
# extension in src/ns-2.35/diffserv/ which completes the dormant
# per-flow lookup hook (policyTableEntry::sourceNode and destNode
# fields have existed since 2000 but were never parsed or matched
# against until 2026). See src/ns-2.35/CHANGELOG.md.
#
# Why a separate file from scenario-2-ns235.tcl: it compares `Dumb`
# (port-based / DSCP-only) against `srTCM` (per-flow rate-metered)
# marking on the same WebTraf topology. Output traces go into a
# disjoint directory so sweeps cannot overwrite
# each other.
#
...
#
# Usage:
#   ns scenario-2-ns235.tcl [paramSet [testTime]]
...
puts "scenario-2-ns235-srtcm: paramSet=$paramSet testTime=${testTime}s"
...
# ---------------------------------------------------------------------------
set outDir "output/ns2-35/example-2-fullscale-srtcm/set-$paramSet"
file mkdir $outDir
...
# === srTCM per-flow classifier (2026-04-18) =================================
# Per-flow CIR/CBS parameters:
# The `addMarkRule` calls above pre-stamp each packet with an app-type
# DSCP that doubles as the PER-FLOW ENTRY LOOKUP KEY. The srTCM rule
# then outputs green/yellow/red based on per-flow bucket state:
#
#   Traffic  ArrivalDSCP  green / yellow / red   CIR(bps)  CBS(bytes)
#   -------  -----------  --------------------   --------  ----------
#   Telnet        10         10 / 10 / 12          50 000      6 250
#   FTP           12         12 / 12 / 14         160 000     20 000
#   HTTP          14         10 / 12 / 14           6 375        797
#
# Telnet and HTTP both produce green=DSCP10 but via DIFFERENT arrival
# DSCPs (10 vs 14), so the per-flow entries live in separate policyTable
# rows and the (greenCP, yellowCP, redCP) triple stored on each entry
# drives the mark-up independently. This is the whole point of the
# per-flow extension — the 2001 DS4 architecture
# cannot express this via the shared policerTable because that table is
# keyed on (policy_index, initialCodePt).
#
# Signature:
...
#                            <policyType>
#                            <params...>          ;# srTCM: CIR CBS EBS
set srtcmCbsSeconds 1.0
set telnetCirBps    50000
set ftpCirBps       160000
set httpCirBps      6375

set telnetCbs [expr {($telnetCirBps / 8.0) * $srtcmCbsSeconds}]
set ftpCbs    [expr {($ftpCirBps    / 8.0) * $srtcmCbsSeconds}]
set httpCbs   [expr {($httpCirBps   / 8.0) * $srtcmCbsSeconds}]

# Per-flow policy entries. Iteration order mirrors scenario-2-ns235.tcl's
# traffic-generation loops (Telnet/FTP use ($i%40)+6 -> ($i%50)+46; HTTP
# uses ($i%40)+6 -> ($i%420)+46 for the 420-client spread).
set nTelnet 50
set nFtp    50
set nHttp   400
for {set i 0} {$i < $nTelnet} {incr i} {
    set src [expr {($i % 40) + 6}]
    set dst [expr {($i % 50) + 46}]
    $qE1C addFlowPolicyEntry [$n($src) id] [$n($dst) id] \
        10  10 10 12  srTCM  $telnetCirBps $telnetCbs 0
}
for {set i 0} {$i < $nFtp} {incr i} {
    set src [expr {($i % 40) + 6}]
    set dst [expr {($i % 50) + 46}]
    $qE1C addFlowPolicyEntry [$n($src) id] [$n($dst) id] \
        12  12 12 14  srTCM  $ftpCirBps $ftpCbs 0
}
for {set i 0} {$i < $nHttp} {incr i} {
    set src [expr {($i % 40) + 6}]
    set dst [expr {($i % 420) + 46}]
    $qE1C addFlowPolicyEntry [$n($src) id] [$n($dst) id] \
        14  10 12 14  srTCM  $httpCirBps $httpCbs 0
}
...
# ---------------------------------------------------------------------------
# Traffic 3: HTTP (400 WebTraf sessions, staggered 0.25 s apart)
#
# Thesis-faithful PagePool/WebTraf generator (ns-2.35 only — see
# docs/WEBTRAF_NS235_FINDING.md for why this cannot run on ns-2.29).
#
# Thesis §4.2 parameters:
#   * 400 sessions, thesis-stated 250 pages per session
#   * inter-page idle time ~ Exp(mean = 15 s)   -> $interPageRV (line 44)
#   * object size            ~ ParetoII(12 KB, shape 1.2) -> $objSizeRV (line 46)
#   * 1 object per page      -> $pageSizeRV (Constant 1)
#   * inter-object gap: thesis is silent; use Constant 0.001 s (negligible)
#     as in examples/webtraf-ns235-test/webtraf-test-diffserv.tcl.
#
# Thesis-faithful numPages=250 (BUG-9 root-caused and fixed):
#   Earlier revisions of this scenario used numPages=1000 as a workaround
#   for an upstream ns-2 WebTraf crash at session completion. Phase A
#   investigation (2026-04-19) pinned the root cause: xuanc's 2011
#   `recycle_page_=1` code path in WebTrafSession::~WebTrafSession deletes
#   the scenario's RandomVariable objects via Tcl on session-end, but the
#   idiomatic ns-2 scenario shares ONE RV instance across all sessions.
#   First session to die → shared RV freed → every remaining session
#   SEGVs on its next rvInterPage_->value() call. Fix lives in the DS4
#   src/ns-2.35/webcache/webtraf.cc destructor; numPages is now back at
#   the thesis value of 250.
#
# Topology mapping:
#   * 40 web servers : n(6)..n(45)
#   * 420 web clients: n(46)..n(465)
#
# DiffServ classification:
#   PagePool/WebTraf does not natively stamp app_type on the TCP agents it
#   spawns. The alloc-tcp override below sets PT_HTTP on every TCP agent
#   so the edge-router "any http" mark rule catches the traffic.
#   NOTE: on ns-2.35 with the Task-1d patch, webcache/webtraf.cc already
#   calls set_apptype(PT_HTTP) natively in WebTrafPool::picktcp() and
#   picksink(); this Tcl override is redundant (belt-and-braces) but kept
#   for documentation.
# ---------------------------------------------------------------------------
puts "Creating 400 HTTP WebTraf sessions (thesis-faithful, ns-2.35)..."

# Per-page object count (thesis: 1 object per page)
set pageSizeRV [new RandomVariable/Constant]
$pageSizeRV set val_ 1

# Inter-object gap: thesis silent; use negligible constant (same as smoke test)
set interObjRV [new RandomVariable/Constant]
$interObjRV set val_ 0.001

# Override alloc-tcp so each WebTraf-allocated TCP agent carries PT_HTTP.
# NOTE: ns-2.35 Task-1d patch also stamps PT_HTTP natively in C++; this
# Tcl hook is kept as belt-and-braces and as documentation of intent.
...
# 40 servers: n(6)..n(45)
$pool set-num-server 40
for {set i 0} {$i < 40} {incr i} {
    $pool set-server $i $n([expr $i + 6])
}

# 420 clients: n(46)..n(465)
$pool set-num-client 420
for {set i 0} {$i < 420} {incr i} {
    $pool set-client $i $n([expr $i + 46])
}

# 400 sessions, thesis-faithful numPages=250, staggered 0.25 s apart.
# create-session <sessionIndex> <numPages> <startTime>
#                <interPageRV> <pageSizeRV> <interObjRV> <objSizeRV>
#
# Calling context may set `overrideNumPages` / `overrideNumSessions` for
# diagnostic or smoke runs (kept for regression-testing BUG-9 remains fixed).
if { [info exists overrideNumPages] } {
    set _webtraf_np $overrideNumPages
} else {
    set _webtraf_np 250
}
if { [info exists overrideNumSessions] } {
    set _webtraf_ns $overrideNumSessions
} else {
    set _webtraf_ns 400
}
$pool set-num-session $_webtraf_ns
for {set i 0} {$i < $_webtraf_ns} {incr i} {
    set startTime [expr $i * 0.25]
    $pool create-session $i $_webtraf_np $startTime \
        $interPageRV $pageSizeRV $interObjRV $objSizeRV
}
...
# Emit per-DSCP TCP retx-byte counters at end-of-sim in a
# "Retx Statistics" block for downstream analysis.
# The C++ counters TCPbReTX_CP[] / TCPbGoTX_CP[] in src/ns-2.35/diffserv/dsred.cc
# accumulate KB; multiply by 1024 to get bytes.
proc print_retx_stats {} {
    global qE1C
    puts ""
    puts "Retx Statistics"
    puts "======================================="
    puts " CP   origBytes   retxBytes  goodput"
    puts " --   ---------   ---------  -------"
    foreach dscp {0 10 12 14 50} {
        set goKB  [$qE1C getStat TCPbGoTX $dscp]
        set retxKB [$qE1C getStat TCPbReTX $dscp]
        set origB  [expr {int($goKB * 1024)}]
        set retxB  [expr {int($retxKB * 1024)}]
        set denom  [expr {$origB + $retxB}]
        if {$denom > 0} {
            set gp [expr {$origB * 1.0 / $denom}]
            puts [format " %2d   %9d   %9d  %7.3f" $dscp $origB $retxB $gp]
        }
    }
    puts "----------------------------------------"
}
$ns at [expr $testTime - 0.05] "print_retx_stats"
