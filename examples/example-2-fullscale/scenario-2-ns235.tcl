...
#
# DISCLAIMER: This is the ns-2.35 variant of thesis Scenario 2
# (Section 4.2 — "full-scale web traffic scenario"). It is a minimal-diff
# fork of scenario-2.tcl whose sole purpose is to replace the 400-stream
# bulk-TCP HTTP approximation with a thesis-faithful PagePool/WebTraf
# traffic generator and to redirect the output directory to
# output/ns2-35/... so ns-2.29 and ns-2.35 runs stay side-by-side.
#
# Why a separate file: PagePool/WebTraf crashes on ns-2.29 (hdr_cmn
# struct growth + a buggy OFFSET macro in webtraf.cc). On ns-2.35 the
# crash is resolved (see docs/WEBTRAF_NS235_FINDING.md) so ns-2.35 can
# drive the thesis Scenario 2 with the original WebTraf model while
# ns-2.29 continues to use the bulk-TCP approximation in scenario-2.tcl.
#
...
puts "scenario-2-ns235: paramSet=$paramSet testTime=${testTime}s"
...
# ---------------------------------------------------------------------------
set outDir "output/ns2-35/example-2-fullscale/set-$paramSet"
file mkdir $outDir
