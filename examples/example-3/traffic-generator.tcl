...
proc video_connection {SourceId SinkId src dst flowId start stop} {
	global ns Sink_
 
        ## initialize a trace file
        set trace_file_($SourceId) [new Tracefile]
        $trace_file_($SourceId) filename example-trace
        ...
        set trace_($SourceId) [new Application/Traffic/Trace]
        $trace_($SourceId) attach-tracefile $trace_file_($SourceId)
   
        $trace_($SourceId) attach-agent $udp_($SourceId)
	...
	$ns attach-agent $dst  $Sink_($SinkId)
	$ns connect $udp_($SourceId) $Sink_($SinkId)
	$ns at $start "$trace_($SourceId) start"
	$ns at $stop  "$trace_($SourceId) stop"
}
