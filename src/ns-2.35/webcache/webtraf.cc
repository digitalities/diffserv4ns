...
WebTrafSession::~WebTrafSession()
{
	...
	// by name to every session's create-session, so all sessions alias the
	// same RV. The first session to complete its last page therefore calls
	// Tcl delete on the shared RV, and every other live session crashes on
	// the next rvInterPage_->value() call (typically inside donePage's
	// sched(rvInterPage_->value()) path).
	//
	// Removing the per-session Tcl-delete block is safe: the Tcl interpreter
	// garbage-collects the RV objects at simulation exit anyway. Scenarios
...
TcpAgent* WebTrafPool::picktcp()
{
	...
	if (a == NULL) {
		...
		nTcp_--;
	// DiffServ4NS: tag WebTraf TCP agents with PT_HTTP for classification (NS2_PATCHES.md §5)
