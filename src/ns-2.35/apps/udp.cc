...
// ns-2.35 port: UdpAgent includes IP+UDP header overhead (20+8 bytes) so
// hdr_cmn::size() matches real-world serialisation. See docs/email-ns2-udp-header-issue.md.
...
void UdpAgent::sendmsg(int nbytes, AppData* data, const char* flags)
{
	...
	while (n-- > 0) {
		p = allocpkt();
		hdr_cmn::access(p)->size() = size_ + IP_HDR_SIZE + UDP_HDR_SIZE;
	...
	if (n > 0) {
		p = allocpkt();
		hdr_cmn::access(p)->size() = n + IP_HDR_SIZE + UDP_HDR_SIZE;
		hdr_rtp* rh = hdr_rtp::access(p);
		rh->flags() = 0;
		rh->seqno() = ++seqno_;
		hdr_cmn::access(p)->timestamp() =
