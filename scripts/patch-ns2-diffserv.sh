...
# 4. The ns-2 binary must be rebuilt after patching (use build-ns2-allinone-docker.sh)
#
...
# - docs/NS2_PATCHES.md      — Documents every modification
#
# Target:
...
if [ ! -d "$NS2_TARGET" ]; then
    ...
    echo "Run ./scripts/fetch-ns2-allinone.sh first." >&2
    exit 1
fi
...
# See docs/NS2_PATCHES.md for the full catalogue of changes.
...
PATCHED_FILES=(
    ...
    "tools/cbr_traffic.cc"  # sets pkttype (note: has a known bug, see NS2_PATCHES.md)
...
echo ""
echo "Next steps:"
echo "  1. Rebuild ns-2: ./scripts/build-ns2-allinone-docker.sh"
