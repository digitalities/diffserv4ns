...
STEPS=(
    ...
    # The tree is already fetched, patched, and built by the time this step
    # runs (the pre-flight above requires it), so drive the matrix and plot
    # directly. The aqm-eval "reproduce" subcommand would re-run the
    # fetch-ns3.sh setup, which is redundant here.
