...
* measured_goodput (when available): TCP-layer retransmission-bytes
  ...
  from the "Retx Statistics" block emitted by ns-2.x with
...
# Retx Statistics block emitted by `getStat TCPbReTX` (ns-2.x).
# Format (whitespace-aligned, fixed-width):
...
def compute_metrics(stats):
    ...
    Retx Statistics block (ns-2.x with retx Tcl
...
def main():
    ...
    # always been tolerance-checked. measured_goodput is directly
    # comparable to thesis_goodput because both are the TCP retx-bytes
    # ratio per DSCP — include it when present.
    ...
    if not has_goodput:
        ...
        print('found in this sweep. Run with the')
