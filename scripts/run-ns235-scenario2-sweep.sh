#!/usr/bin/env bash
# run-ns235-scenario2-sweep.sh — 6-set WRED sweep for scenario-2-ns235.tcl
#
# Runs thesis-faithful PagePool/WebTraf Scenario 2 on ns-2.35 for all six
# WRED parameter sets. Each set's stdout lands in
# output/ns2-35/example-2-fullscale/set-$i/run.log and the three trace
# files (ServiceRate.tr, QueueLen.tr, PktLoss.tr) beside it.
#
# Usage:
#   scripts/run-ns235-scenario2-sweep.sh                    # 6 sets, 5000 s each
#   SETS="1"       scripts/run-ns235-scenario2-sweep.sh     # subset of sets
#   SIM_TIME=100   scripts/run-ns235-scenario2-sweep.sh     # shortened smoke
#
...
OUT_BASE="output/ns2-35/example-2-fullscale"
mkdir -p "$OUT_BASE"

echo "=== ns-2.35 Scenario 2 WebTraf sweep ==="
echo "Sets      : $SETS"
...
for PARAM_SET in $SETS; do
    ...
    cp "$EXAMPLE_DIR"/scenario-2-ns235.tcl  "$WORKDIR/"
    ...
    # The scenario writes traces to output/ns2-35/example-2-fullscale/set-$N/,
    # resolved relative to the working directory. Pre-create the tree in the
    # workdir so `file mkdir` inside the scenario lands there.
    mkdir -p "$WORKDIR/output/ns2-35/example-2-fullscale/set-$PARAM_SET"
    ...
    docker run --rm \
        ...
        ubuntu:18.04 \
        /ns-allinone/ns-2.35/ns scenario-2-ns235.tcl "$PARAM_SET" "$SIM_TIME" \
        2>&1 | tee "$OUTDIR/run.log" || RC=${PIPESTATUS[0]:-$?}

    # Harvest trace files.
    TR_FOUND=0
    for TR in ServiceRate.tr QueueLen.tr PktLoss.tr; do
        SRC="$WORKDIR/output/ns2-35/example-2-fullscale/set-$PARAM_SET/$TR"
        if [ -f "$SRC" ]; then
            cp "$SRC" "$OUTDIR/"
            TR_FOUND=$((TR_FOUND + 1))
        fi
